...
export const createProject = async (req, res) => {
    ...
    // 2. Check if user is ADMIN of this workspace
    if(!workspace.members.some((member) => member.userId === userId && member.role === "ADMIN")){
         return res.status(403).json({ message: "You dont have permission to create project in this woekspace" });
    }
        ...
        const { userId} = await req.auth();
         ...
         // check if user is project lead
          ...
          if(project.team_lead !== userId){
            return res.status(403).json({ message: "only project lead can add member to this project" });
          }
